  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={markAllRead} className="text-blue-700 underline">{t('mark_all_read')}</button>
      </div>
      ...
      <div className="space-y-3">
        {notifications.map(notif => (
          <div key={notif.id} className={`p-4 border rounded shadow-sm ${notif.read ? 'bg-gray-50' : 'bg-white border-l-4 border-l-blue-600'}`}>
            ...
            {!notif.read && <button onClick={() => markAsRead(notif.id)} className="text-blue-600 text-sm mt-2">{t('mark_as_read')}</button>}
          </div>
        ))}
      </div>
    </div>
